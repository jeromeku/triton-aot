...
from .utils import (
    InfoFromBackendForTensorMap,
    TensorMapManager,
    get_ids_of_tensormaps,
    parse_tma_info,
)
...
def ttgir_to_llir(mod, extern_libs, target, tma_infos):
    ...
    if _is_cuda(target):
        return translate_triton_gpu_to_llvmir(
            mod, target.capability, tma_infos, runtime.TARGET.NVVM
        )
    else:
...
def llir_to_ptx(mod: Any, target: CudaTargetDescriptor, ptx_version: int = None) -> str:
    ...
    return translate_llvmir_to_ptx(
        mod, target.capability, ptx_version, target.enable_fp_fusion
    )
...
def compile(fn, **kwargs):
    ...
    assert (
        num_warps > 0 and (num_warps & (num_warps - 1)) == 0
    ), "num_warps must be a power of 2"
    num_ctas = kwargs.get("num_ctas", 1)
    num_stages = kwargs.get(
        "num_stages", get_arch_default_num_stages(device_type, capability=capability)
    )
    ...
    if is_cuda:
        ...
        _device_backend.add_stages(
            target, extern_libs, stages, num_warps=num_warps, num_stages=num_stages
        )
    else:
    ...
    if isinstance(fn, JITFunction):
        ...
        if ir_name == "ttgir":
            ...
            assert (
                len(num_warps_matches) == 1
            ), "Expected exactly one match for num_warps"
            assert (
    ...
    fn_override_manager = get_override_manager(
        make_hash(
            fn, target, get_env_vars(), _device_backend, **kwargs, ignore_version=True
        )
    )
        ...
        if ir_name == ext:
            ...
            if path is None:
                ...
                if ir_name == "amdgcn":
                    ...
                    assert (
                        hasco_path is not None
                    ), "Expected to have hsaco_path in metadata when we have the amdgcn"
    ...
    ids_of_folded_args = (
        tuple([int(k) for k in configs[0].ids_of_folded_args])
        if isinstance(fn, JITFunction)
        else ()
    )
...
class CompiledKernel:
    ...
    def __init__(self, fn, so_path, metadata, asm):
        ...
        self.device_backend = (
            get_backend(self.device_type) if self.device_type not in ["cuda"] else None
        )
        # initialize asm dict
        self.asm = asm
    ...
    def _init_handles(self):
        ...
        if self.device_type in ["cuda"]:
            ...
            max_shared = self.device_backend.get_device_properties(device)[
                "max_shared_mem"
            ]
        ...
        mod, func, n_regs, n_spills = fn_load_binary(
            self.metadata["name"], self.asm[bin_path], self.shared, device
        )
    ...
    def assemble_tensormap_to_arg(self, args):
        ...
        if hasattr(self, "tensormaps_info"):
            # tuple for hashable
            args_ptr = tuple(
                [arg.data_ptr() if hasattr(arg, "data_ptr") else arg for arg in args]
            )
